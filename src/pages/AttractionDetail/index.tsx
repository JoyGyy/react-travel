...
export default function AttractionDetail() {
  ...
  if (loading) return <main className="attraction-detail"><Spin /> 加载景点详情中...</main>
  if (error || !attraction) return <main className="attraction-detail" role="alert"><p>{error || '景点不存在或已下架'}</p><Button onClick={() => navigate('/attractions')}>返回景点列表</Button></main>
  ...
  return (
    <main className="attraction-detail" aria-labelledby="attraction-detail-title">
      {contextHolder}
      <button type="button" className="attraction-detail__back" onClick={() => navigate(-1)}><ArrowLeftOutlined /> 返回</button>
      ...
      <section className="attraction-detail__section"><h2>景点介绍</h2><p>{attraction.description}</p></section>
      <section className="attraction-detail__section"><h2>实用信息</h2><p>地址：{attraction.address}</p><p>开放时间：{attraction.openingHours}</p><p>建议游玩：{attraction.recommendedDuration}</p></section>
      <section className="attraction-detail__section"><h2>游玩亮点</h2><ul>{attraction.highlights.map(item => <li key={item}>{item}</li>)}</ul></section>
      <section className="attraction-detail__section"><h2>注意事项</h2><ul>{attraction.tips.map(item => <li key={item}>{item}</li>)}</ul></section>
      <section className="attraction-detail__section"><h2>购票入口</h2><p className="attraction-detail__notice">价格、库存和开放时间以第三方平台及景区官方公告为准。</p><div className="attraction-detail__booking">{buildBookingLinks(attraction).map(link => <a key={link.key} href={link.href} target="_blank" rel="noopener noreferrer">去{link.label}查看</a>)}</div></section>
