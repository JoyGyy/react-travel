...
import type { SearchResult, CityKnowledge } from './rag.js'
...
async function executeTool(toolName: string, args: Record<string, unknown>, context: AgentContext): Promise<ToolResult> {
  switch (toolName) {
    case 'parse_intent': {
      const intent: AgentIntent = {
        city: (args.city as string) || '北京',
        days: Math.max(1, Math.min(Number(args.days) || 3, 10)),
        budget: Math.max(1000, Number(args.budget) || 5000),
        tags: (args.tags as string[]) || ['必去'],
      }
    ...
    case 'search_attractions': {
      const city = (args.city as string) || context.intent?.city || '北京'
      const tags = (args.tags as string[]) || context.intent?.tags || []
    ...
    case 'plan_itinerary': {
      const itinerary = (args.itinerary as DayItinerary[]) || []
    ...
    case 'calculate_budget': {
      const totalBudget = (args.totalBudget as number) || context.intent?.budget || 5000
      const ticketCost = (args.ticketCost as number) || 0
      ...
      return { output: JSON.stringify(budgetBreakdown), summary: budgetBreakdown as unknown as Record<string, unknown> }
    }
    ...
    case 'generate_tips': {
      ...
      if ((args.food as string[])?.length)
        tips.push(`推荐美食：${(args.food as string[]).slice(0, 5).join('、')}`)
      if ((args.days as number) >= 3)
...
function formatSpot(spot: { name: string, description: string, duration?: string, ticket?: number, transportation?: string } | undefined): SpotData {
  if (!spot)
 ...
 * 晚间加入餐饮推荐
 */
function buildWeatherAwareItinerary(context: AgentContext): DayItinerary[] {
  ...
  // 根据天气筛选景点排序
  let sortedSpots: typeof allSpots
  if (goodOutdoor) {
    // 晴天：户外优先，室内穿插
...
async function executeAgent(res: Response, params: AgentParams): Promise<void> {
  ...
  const context: AgentContext = { weather }
  ...
  try {
    for (let round = 0; round < 10; round++) {
      ...
      messages.push({ ...assistantMsg, content: assistantMsg.content ?? undefined } as ChatMessage)
    ...
    if (!context.intent) {
      ...
      const result = await executeTool('parse_intent', params as unknown as Record<string, unknown>, context)
    ...
    if (!context.tips) {
      ...
      const cityData = context.ragResult || ({} as Partial<SearchResult>)
    ...
    log.error('Agent 执行失败，降级到 Mock:', (err as Error).message)
    return executeAgentMock(res, params)
  }
}
...
async function executeAgentMock(res: Response, params: AgentParams): Promise<void> {
  // 步骤 1：解析意图
  sendSSE(res, { type: 'step', step: 1, name: '解析意图', status: 'start' })
  const tags: string[] = []
  ...
  sendSSE(res, { type: 'step', step: 4, name: '行程规划', status: 'start' })
  const goodOutdoor = isGoodForOutdoor(weather)
  let sortedSpots: typeof ragResult.attractions
  if (goodOutdoor) {
